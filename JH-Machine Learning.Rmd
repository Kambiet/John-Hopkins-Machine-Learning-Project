...
```{r warning=FALSE, error=FALSE}
setwd("C:/Users/Perez.KAMBIET/OneDrive - Université Mohammed VI Polytechnique/Documents")
```  
...
# Ensure the model is created or loaded
if (!exists("modelTree")) {
...
predictTree <- factor(predictTree, levels = levels(validation$classe))

# Check levels
if (!all(levels(predictTree) %in% levels(validation$classe))) {
    stop("Levels of predictTree do not match levels of validation$classe")
}

# Create the confusion matrix
confusionMatrix <- confusionMatrix(validation$classe, predictTree)
...
```{r warning=FALSE, error=FALSE}
# Generate predictions
predictions <- predict(modelRF, testing[, -length(names(testing))])

# Define the function to write files
